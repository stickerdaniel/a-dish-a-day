...
struct ZigZagLineView<Content: View>: View {
    ...
    init(offsetYValues: [CGFloat], views: [Content], lineHeight: CGFloat = 200, maxWidth: CGFloat = 900) {
            ...
            ZStack {
                ...
                Path { path in
                    ...
                    for index in 0..<views.count {
                        ...
                        let previousOffsetY = previousIndex >= 0
                            ? offsetYValues[previousIndex]
                            : 0
                        let previousOffsetX = previousIndex >= 0
                            ? spacing * (previousIndex % 2 == 0 ? 1 : -1)
                            : 0
                        
                        // Introduce variation based on index (every other point or pattern-based)
                        let inwardOffsetFactor: CGFloat = (index % variationIndex == variationIndex-1)
                            ? variationFactor
                            : 1.0
                        
                        let previousInwardOffsetFactor: CGFloat = (previousIndex % variationIndex == variationIndex-1)
                            ? variationFactor
                            : 1.0
