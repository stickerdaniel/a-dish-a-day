...
struct CalendarCard: View {
  ...
  var body: some View {
    ...
    .contextMenu {
      ...
      if calendar.source == .imported {
        Button(action: copyToCreatedSection) {
          Label(
            "Copy to \(CalendarTab.created.rawValue)",
    ...
    .alert(
      "All recipes must be unlocked to copy and edit this calendar.", isPresented: $showUnlockAlert
    ) {
  ...
  private func copyToCreatedSection() {
    ...
    let copiedCalendar = CalendarModel.copy(
      from: calendar, setName: calendar.name + " (imported copy)",
